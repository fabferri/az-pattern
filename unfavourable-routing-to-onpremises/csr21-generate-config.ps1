#####
##### Script to generate the IPSec configuration of Cisco CSR1 in siteB
#####
#####
...
######################### Azure
$subscriptionName= $json.siteB.subscriptionName  ### name of the Azure subscription with cisco CSR
$rg_csr_Local = $json.siteB.rgName               ### resource group of the remote CSR
$csrName_Local= $json.siteB.csr1_vmName          ### Name of the local CSR
$pubIPName_csr_Local= $csrName_Local+"-pubIP"    ### Name of public IP assigned to local CSR

$rg_csr_Remote=  $json.siteA.rgName              ### resource group of the remote CSR
$csrName_Remote= $json.siteA.csr1_vmName         ### Name of the remote CSR
...
$fileName="csr21-iosxe-cfg.txt"         # filename of output txt file with CSR config
######################### PARAMETERS CSR remote
$remoteASN="65001"                      # remote CSR-BGP ASN assigned to the VPN gateway: it is fixed do not changed it!!!
$pubIP_csrRemote=$IPcsr_Remote          # remote CSR-public IP address-instance0 of the VPN Gateway
$remotePeerBGP0="172.16.1.1"            # remote CSR-BGP peer-instance0 of the VPN Gateway
$privIP_csrRemote ="10.0.1.10"          # remote CSR-private ip address external interface
$PSK="secret*!*PSK101"                  # shared secret site-to-site VPN

######################### PARAMETERS CSR local
$localASN="65002"                       # CSR-BGP ASN
$ip_loopback="172.16.1.2"               # CSR-IP address loopback interface (without SUBNETMASK)
$mask_loopback="255.255.255.255"        # CSR-subnet mask lookback interface (do not change the SUBNETMASK)
$ip_Tunnel0="192.168.0.2"               # CSR-IP ADDRESS of the tunnel0 interface
...
$priv_externalGateway="10.0.2.1"        # CSR-IP default gateway of the subnet attached to the EXTERNAL NIC
$priv_internalNetw1="10.0.1.96"         # CSR-private NETWORK PREFIX assigned to the INTERNAL NIC
$mask_internalNetw1="255.255.255.224"   # CSR-SUBNET MASK of private network assigned to the INTERNAL NIC
$priv_internalNetw2="10.0.1.128"        # CSR-private NETWORK PREFIX assigned to the INTERNAL NIC
$mask_internalNetw2="255.255.255.224"   # CSR-SUBNET MASK of private network assigned to the INTERNAL NIC
$priv_internalNextHopIP1="10.0.2.33"
#####
...
!
ip route $priv_internalNetw1 $mask_internalNetw1 $priv_internalNextHopIP1
ip route $priv_internalNetw2 $mask_internalNetw2 $priv_internalNextHopIP1
!
line vty 0 4
 exec-timeout 20 0
exit
