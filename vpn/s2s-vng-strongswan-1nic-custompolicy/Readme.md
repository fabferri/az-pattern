<properties
   pageTitle="Site-to-Site VPN between strongSwan and Azure VPN Gateway"
   description="Site-to-Site VPN between strongSwan and Azure VPN Gateway"
...
# Site-to-Site VPN between strongSwan and Azure VPN Gateway with Custom IPsec/IKE Policy (GCMAES256)
...
- [Annex: Single-NIC routing for IPsec](#annex-single-nic-routing-for-ipsec)
...
| File | Description |
|------|-------------|
| `init.json` | Input parameters: subscription, resource group, location, VM credentials |
| `01_azvpn.json` | ARM template for Azure infrastructure (VNets, VPN Gateway, NVA VM, VMs, NSGs, route tables) |
| `01_azvpn.ps1` | PowerShell script to deploy `01_azvpn.json` |
| `02_vpnconnections.json` | ARM template for local network gateways and VPN connections with custom IPsec/IKE policy |
| `02_vpnconnections.ps1` | PowerShell script to deploy `02_vpnconnections.json` |
| `03_collect-values.ps1` | Collects deployment outputs (public IPs, BGP IPs, shared key) and generates NVA configuration values |
| `configure-nva.sh` | Bash script to configure the NVA: installs strongSwan, FRR, creates XFRM interfaces, sets up BGP |
...
| **vnet2** (10.2.0.0/24) | NVA (Ubuntu 24.04, strongSwan, single NIC, ASN 65020) + vm2 |
...
| NIC | Subnet | Private IP | Purpose |
|-----|--------|-----------|--------|
| eth0 | nvaSubnet (10.2.0.0/27) | 10.2.0.10 | Management (SSH) + IPsec tunnel endpoint + transit traffic (single public IP) |
...
This deploys: vnet1, vnet2, VPN Gateway (active-active), NVA VM (single NIC), vm1, vm2, NSGs, and route tables.
...
FRR's `network <prefix>` command only originates a BGP advertisement if the **exact prefix** exists in the kernel routing table (RIB). The NVA has connected routes for `/27` subnets (nvaSubnet + subnetApp), but not for the aggregate `10.2.0.0/24`. Without an explicit RIB entry, BGP will silently refuse to advertise the prefix.
...
## Annex: Single-NIC routing for IPsec

With a single NIC, routing is straightforward — no policy routing or multi-NIC considerations are needed:

### Inbound IPsec traffic (VPN Gateway → NVA)

Azure VPN Gateway sends IKE/ESP packets to the NVA's public IP (`nva-pubIP`). This public IP is bound to eth0's IP configuration in the ARM template. Azure fabric translates `nva-pubIP → 10.2.0.10` and delivers the packet directly to eth0.

### Outbound IPsec traffic (NVA → VPN Gateway)

The `vpn-xfrm-setup.sh` script adds explicit host routes:
```bash
ip route add <VPNGW_PIP0>/32 via 10.2.0.1
ip route add <VPNGW_PIP1>/32 via 10.2.0.1
```
Gateway `10.2.0.1` is in eth0's connected subnet (10.2.0.0/27). Azure then NATs the source from `10.2.0.10 → nva-pubIP`, which is what the VPN Gateway expects as the peer address.

### Why single NIC simplifies routing

1. **No multi-NIC routing conflicts** — With one NIC, all traffic (management, IPsec, transit) uses the same interface and default gateway.
2. **Azure DHCP configures the default route on eth0** — Since it's the only NIC, it gets both the connected subnet route and default gateway.
3. **strongSwan binds to the correct source** — `local_addrs = 10.2.0.10` in swanctl.conf matches eth0's private IP; Azure's NAT translates it to the public IP that the VPN Gateway knows.
4. **Explicit host routes for VPN Gateway** — The setup script adds `/32` routes for VPN Gateway public IPs to ensure traffic goes via the subnet gateway (not through the tunnel itself, avoiding routing loops).
