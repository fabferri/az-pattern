...
###############################################################################
# 3. NVA public IP (single NIC: management + IPsec endpoint)
###############################################################################
try {
...
###############################################################################
# 4. NVA private IP (single NIC)
###############################################################################
try {
    Write-Host "  Querying NVA NIC private IP: $nvaNicName" -ForegroundColor Gray
    $nvaPrivIP = az network nic show `
        --resource-group $resourceGroupName `
        --name $nvaNicName `
        --query "ipConfigurations[0].privateIPAddress" --output tsv
    if ($LASTEXITCODE -ne 0 -or !$nvaPrivIP) { throw "Failed to get NIC private IP: $nvaNicName" }
}
catch {
...
###############################################################################
# 6. NVA SSH IP (same as NVA public IP in single-NIC design)
###############################################################################
...
Write-Host "  NVA_PRIVATE_IP.........: $nvaPrivIP" -ForegroundColor Yellow
...
try {
    ...
    $replacements = @(
        @{ Pattern = '(NVA_PRIVATE_IP=)"[^"]*"';           Value = "`${1}`"$nvaPrivIP`"" }
