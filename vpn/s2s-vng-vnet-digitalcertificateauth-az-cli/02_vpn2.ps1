...
If ($keyVault -eq $null) {
    ...
    $deletedKeyVault = Get-AzKeyVault -VaultName $keyVault2Name -Location $location -InRemovedState -ErrorAction SilentlyContinue
    if ($deletedKeyVault) {
        Write-Host (Get-Date)"- Key Vault '$keyVault2Name' found in soft-deleted state, purging..." -ForegroundColor Cyan
        Remove-AzKeyVault -VaultName $keyVault2Name -Location $location -InRemovedState -Force
        Write-Host (Get-Date)"- Key Vault '$keyVault2Name' purged successfully" -ForegroundColor Cyan
    }
