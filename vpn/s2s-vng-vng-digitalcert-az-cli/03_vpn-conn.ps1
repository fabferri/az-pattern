...
# Check if both VPN gateways have been deployed successfully
Write-Host (Get-Date)"- checking if VPN gateways have been deployed successfully..." -ForegroundColor Green

try {
    $gw1 = Get-AzVirtualNetworkGateway -Name $gw1Name -ResourceGroupName $rgName -ErrorAction Stop
    if ($gw1.ProvisioningState -ne 'Succeeded') {
        Write-Host (Get-Date)"- VPN Gateway1 '$gw1Name' provisioning state is '$($gw1.ProvisioningState)', expected 'Succeeded'" -ForegroundColor Red
        Write-Host (Get-Date)"- Please wait for the gateway deployment to complete before running this script" -ForegroundColor Yellow
        Exit
    }
    Write-Host (Get-Date)"- VPN Gateway1 '$gw1Name' is deployed successfully (ProvisioningState: $($gw1.ProvisioningState))" -ForegroundColor Cyan
}
catch {
    Write-Host (Get-Date)"- VPN Gateway1 '$gw1Name' not found. Please deploy the gateway first using 01_vpn1.ps1" -ForegroundColor Red
    Exit
}

try {
    $gw2 = Get-AzVirtualNetworkGateway -Name $gw2Name -ResourceGroupName $rgName -ErrorAction Stop
    if ($gw2.ProvisioningState -ne 'Succeeded') {
        Write-Host (Get-Date)"- VPN Gateway2 '$gw2Name' provisioning state is '$($gw2.ProvisioningState)', expected 'Succeeded'" -ForegroundColor Red
        Write-Host (Get-Date)"- Please wait for the gateway deployment to complete before running this script" -ForegroundColor Yellow
        Exit
    }
    Write-Host (Get-Date)"- VPN Gateway2 '$gw2Name' is deployed successfully (ProvisioningState: $($gw2.ProvisioningState))" -ForegroundColor Cyan
}
catch {
    Write-Host (Get-Date)"- VPN Gateway2 '$gw2Name' not found. Please deploy the gateway first using 02_vpn2.ps1" -ForegroundColor Red
    Exit
}

Write-Host (Get-Date)"- Both VPN gateways are deployed and ready" -ForegroundColor Green
