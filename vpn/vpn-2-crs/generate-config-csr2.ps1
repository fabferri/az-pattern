...
try {
  $csr1privIP=(Get-AzNetworkInterface -Name $csr1_nic0Name -ResourceGroupName $rgName).IpConfigurations[0].PrivateIpAddress
  $csr2privIP=(Get-AzNetworkInterface -Name $csr2_nic0Name -ResourceGroupName $rgName).IpConfigurations[0].PrivateIpAddress
}
catch {
  write-host "csr-public IPs not found:" -ForegroundColor Yellow 
  Exit
...
$remotecsr1ASN = '65001'
$remotecsr1PubIP1 = $csr1PubIP
$remotecsr1privIP1 = $csr1privIP
######################### PARAMETERS Cisco CSR
$PSK = $sharedKey                        # VPN GTW VPN site-so-site shared secret
$localASN = '65002'                      # CSR-BGP ASN
$ip_loopback = '192.168.2.1'             # CSR-IP address loopback interface (without SUBNETMASK)
$mask_loopback = '255.255.255.255'       # CSR-subnet mask lookback interface (do not change the SUBNETMASK)

$ip_Tunnel1 = '172.16.0.2'               # CSR-IP ADDRESS of the tunnel0 interface
$mask_Tunnel1 = '255.255.255.255'        # CSR-SUBNETMASK of the tunnel0 interface (do not change the SUBNETMASK)


$priv_externalNetw = '10.0.2.0'          # CSR-private NETWORK PREFIX assigned to the EXTERNAL NIC
$mask_externalNetw = '255.255.255.224'   # CSR-mask of private network assigned to the EXTERNAL NIC
$priv_defGtwExternal = '10.0.2.1'        # CSR-IP default gateway of the subnet attached to the EXTERNAL NIC0

$priv_internalNetw1 = '10.0.2.32'        # CSR-private NETWORK PREFIX assigned to the INTERNAL NIC
$mask_internalNetw1 = '255.255.255.224'  # CSR-SUBNET MASK of private network assigned to the INTERNAL NIC
$priv_defGtwInternal = '10.0.2.33'
$priv_internalNetw2 = '10.0.2.64'        # private NETWORK PREFIX assigned to the internal subnet not direct connected to the CSR
$mask_internalNetw2 = '255.255.255.224'  # private MAS assigned to the internal subnet not direct connected to the CSR
$priv_internalNetw3 = '10.0.2.96'        # private NETWORK PREFIX assigned to the internal subnet not direct connected to the CSR
...
write-host "csr2-BGPPeer1...........: "$remotecsr2BGPPeer1 -ForegroundColor Cyan
...
crypto ikev2 profile az-PROFILE1
 ...
 match identity remote address $remotecsr1privIP1 255.255.255.255
...
interface Tunnel1
 ...
 tunnel destination $remotecsr1PubIP1
...
router bgp $localASN
 ...
 neighbor $remotecsr1privIP1 remote-as $remotecsr1ASN
 neighbor $remotecsr1privIP1 ebgp-multihop 3
 neighbor $remotecsr1privIP1 update-source GigabitEthernet1
 !
 address-family ipv4
  ...
  neighbor $remotecsr1privIP1 activate
  neighbor $remotecsr1privIP1 next-hop-self
  neighbor $remotecsr1privIP1 soft-reconfiguration inbound
...
!
!
ip route $remotecsr1privIP1 255.255.255.255 Tunnel1
