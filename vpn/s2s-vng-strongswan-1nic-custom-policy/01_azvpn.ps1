################# Input parameters #################
$deploymentName = 'vnet-gw'
$armTemplateFile = '01_azvpn.json'
...
try {
    ...
    Write-Host "$(Get-Date) - running ARM template: $templateFile - Create vnets, vpng gateway, NVA, VMs..." -ForegroundColor Cyan
    Write-Host "$(Get-Date) - waiting for deployment to complete..." -ForegroundColor Cyan
