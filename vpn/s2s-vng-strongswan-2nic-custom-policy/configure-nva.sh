...
# configure-nva.sh
#
# Configures an Ubuntu 24.04 NVA (dual NIC) as a strongSwan IPsec gateway with
# route-based VPN using XFRM interfaces, BGP via FRR, and reboot-persistent
# configuration.
#
# NIC layout:
#   eth0 = External NIC (nvaExternalSubnet) — SSH management + IPsec endpoint
#   eth1 = Internal NIC (nvaInternalSubnet) — decrypted transit traffic to/from VM2
#
# Usage:
#   1. Deploy Azure infra with 01_azvpn.ps1 and 02_vpnconnections.ps1
#   2. Fill in the variables below from deployment outputs
#   3. SSH to the NVA external public IP and run: sudo bash configure-nva.sh
#
...
# NVA external NIC settings (eth0: SSH + IPsec endpoint)
NVA_EXTERNAL_IP="10.2.0.10"                 # NVA external NIC private IP
NVA_PUBLIC_IP="REPLACE_WITH_NVA_PUBLIC_IP"  # NVA external NIC public IP (Azure-assigned)
NVA_EXTERNAL_IFACE="eth0"                   # NVA external interface name
NVA_EXTERNAL_GW="10.2.0.1"                  # Default gateway for the external subnet

# NVA internal NIC settings (eth1: transit traffic to/from VM2)
NVA_INTERNAL_IP="10.2.0.40"                 # NVA internal NIC private IP
NVA_INTERNAL_IFACE="eth1"                   # NVA internal interface name
NVA_INTERNAL_GW="10.2.0.33"                 # Default gateway for the internal subnet
...
VPNGW_PIP0="REPLACE_WITH_VPNGW_PIP0"        # VPN GW instance 0 public IP
VPNGW_PIP1="REPLACE_WITH_VPNGW_PIP1"        # VPN GW instance 1 public IP
...
VPN_PSK="REPLACE_WITH_SHARED_SECRET"
...
# Additional local subnets (reachable via NVA internal gateway)
SUBNET_APP="10.2.0.64/27"                   # subnetApp — reachable via internal NIC gateway
...
# Create XFRM interface for tunnel 0 (bound to external NIC)
ip link add ipsec0 type xfrm dev ${NVA_EXTERNAL_IFACE} if_id ${XFRM_IF_ID0}
ip addr add ${NVA_BGP_IP0}/32 dev ipsec0
ip link set ipsec0 up

# Create XFRM interface for tunnel 1 (bound to external NIC)
ip link add ipsec1 type xfrm dev ${NVA_EXTERNAL_IFACE} if_id ${XFRM_IF_ID1}
...
# Routes to VPN Gateway public IPs via external NIC gateway
ip route add ${VPNGW_PIP0}/32 via ${NVA_EXTERNAL_GW}
ip route add ${VPNGW_PIP1}/32 via ${NVA_EXTERNAL_GW}
...
# Route to subnetApp via internal NIC gateway (transit traffic)
ip route add ${SUBNET_APP} via ${NVA_INTERNAL_GW} dev ${NVA_INTERNAL_IFACE}
...
ip route del ${VPNGW_PIP0}/32 via ${NVA_EXTERNAL_GW} 2>/dev/null || true
ip route del ${VPNGW_PIP1}/32 via ${NVA_EXTERNAL_GW} 2>/dev/null || true
ip route del throw ${VPNGW_PIP0}/32 table 220 2>/dev/null || true
ip route del throw ${VPNGW_PIP1}/32 table 220 2>/dev/null || true
ip route del ${SUBNET_APP} via ${NVA_INTERNAL_GW} dev ${NVA_INTERNAL_IFACE} 2>/dev/null || true
