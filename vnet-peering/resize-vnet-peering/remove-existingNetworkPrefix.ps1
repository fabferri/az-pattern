# powershell script to remove an existing network prefix from a vnet.
#
# Description of variables:
# $rgName: name of the resource group
# $vnetName1: first vnet. it is the target vnet name we want to remove the network prefix
...
Try { 
        if( $VNet.AddressSpace.AddressPrefixes.Contains($NewNetworkPrefix) ) { Write-Host "$(Get-Date) - address prefix: $NewNetworkPrefix , found in the vnet: $vnetName1" -ForegroundColor Green}
        else { Write-Host "$(Get-Date) - address prefix: $NewNetworkPrefix does not exist in vnet: $vnetName1" -ForegroundColor Yellow; Exit}
    }
...
Try { 
    Write-Host "$(Get-Date) - removing the network prefix: $NewNetworkPrefix  in the vnet: $vnetName1" -ForegroundColor Cyan
    # remove the address space in the vnet
    $VNet.AddressSpace.AddressPrefixes.Remove($NewNetworkPrefix)
    Set-AzVirtualNetwork -VirtualNetwork $VNet
}
Catch {Write-Host "$(Get-Date) - issue to remove the address prefix: $NewNetworkPrefix , in the vnet: $vnetName1"; Exit}
