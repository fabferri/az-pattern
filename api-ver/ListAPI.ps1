...
$apilist3=((Get-AzResourceProvider -ProviderNamespace Microsoft.Network -Location $location).ResourceTypes | Where-Object ResourceTypeName -eq vpnGateways).ApiVersions
Write-host $apilist3 -Separator "`n" -ForegroundColor Green
Write-host "-----------------------------------------" 


Write-host "===================== Azure firewall API:" -ForegroundColor Red
$apilist4=((Get-AzResourceProvider -ProviderNamespace Microsoft.Network -Location $location).ResourceTypes | Where-Object ResourceTypeName -eq azureFirewalls).ApiVersions
Write-host $apilist4 -Separator "`n" -ForegroundColor Red
Write-host "-----------------------------------------" 
