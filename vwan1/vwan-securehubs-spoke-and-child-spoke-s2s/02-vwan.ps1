...
#   $fwhubName: name of the Azure Firewall in the virtual hub
#   $fwPolicy1Name: name of the Azure Firewall policy
...
#   deployVPNGtwS2S: runtime flag computed by the script (not read from init.json)
#
################# Input parameters #################
param(
...
try {
     ...
     $fwhubName = $arrayParams.fwhubName
     $fwPolicy1Name = $arrayParams.fwPolicy1Name
...
if (!$fwhubName) { Write-Host 'variable $fwhubName is null' ; Exit }                           else { Write-Host '   firewall hub name.......: '$fwhubName -ForegroundColor Yellow }
if (!$fwPolicy1Name) { Write-Host 'variable $fwPolicy1Name is null' ; Exit }                   else { Write-Host '   firewall policy name....: '$fwPolicy1Name -ForegroundColor Yellow }
