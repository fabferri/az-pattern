...
try {
     ...
     $hub1vpnGtwName = $arrayParams.hub1vpnGwName
...
if (!$hub1vpnGtwName) { Write-Host 'variable $hub1vpnGtwName is null' ; Exit }       else { Write-Host '   hub1 VPN gateway name...: '$hub1vpnGtwName -ForegroundColor Yellow }
...
try {
     ...
     $deployVPNGtw = $false
     write-host "$(Get-Date) - skipping the creation of S2S VPN GTW in vWAN: $hub1vpnGtwName" -ForegroundColor Cyan
}
catch {
...
Catch { 
     ...
     Set-AzResourceGroup -Name $rgName -Tag @{"PM owner"="fabferri"; "Project" = "vWAN validation"}
}
