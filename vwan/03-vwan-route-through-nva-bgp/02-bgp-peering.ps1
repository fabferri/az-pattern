#
# Variables defined in init.json file:
# $adminUsername
# $adminPassword
# $subscriptionName
# $ResourceGroupName
...
$armTemplateFile = '02-bgp-peering.json'
...
Try {
  $routingStatehub1=(Get-AzVirtualHub -ResourceGroupName $rgName -Name $hub1Name).RoutingState
  if ($routingStatehub1.Equals( "Provisioned")) { Write-Host  "Routing state of $hub1Name :"$routingStatehub1 -ForegroundColor Cyan} 
  else {Write-Host  "Routing state of $hub1Name :"$routingStatehub1; write-host "wait the virtual router is changed in Provisioned"; Exit}
}
catch { Write-Host "issue in getting Routing state of $hub1Name" ;Exit}


Try {
     $routingStatehub2=(Get-AzVirtualHub -ResourceGroupName $rgName -Name $hub2Name).RoutingState
     if ($routingStatehub2.Equals( "Provisioned")) { Write-Host  "Routing state of $hub2Name :"$routingStatehub2 -ForegroundColor Cyan} 
     else {Write-Host  "Routing state of $hub2Name :"$routingStatehub2; write-host "wait the virtual router is changed in Provisioned"; Exit}
   }
catch { Write-Host "issue in getting Routing state of $hub2Name" ; Exit}
