...
try {
  $hubBGPPeer1=(Get-AzVirtualHub -ResourceGroupName $rgName -Name $hub2Name).VirtualRouterIps[0]
  $hubBGPPeer2=(Get-AzVirtualHub -ResourceGroupName $rgName -Name $hub2Name).VirtualRouterIps[1]
}
catch{
  write-host "hub2-BGP peering addresses not found:" -ForegroundColor Yellow 
  Exit
}
...
####
$fileName="csr2-add-bgp-config.txt"     # filename of output txt file with CSR config

write-host ""
write-host "Remote hub2-BGP ASN...............: $hubRemoteASN" -ForegroundColor Cyan
write-host "Remote hub2-BGP peer address1.....: $hubBGPPeer1" -ForegroundColor Cyan
write-host "Remote hub2-BGP peer address2.....: $hubBGPPeer2" -ForegroundColor Cyan
