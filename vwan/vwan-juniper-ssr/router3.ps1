...
$armTemplateFile = 'router3.json'
$deploymentName = 'router3'
$conductorPubIPName = 'conductor-control'
####################################################
...
<#
# Acquire the public IP address of the conductor
Try { 
     Get-AzPublicIpAddress -ResourceGroupName $rgName -Name $conductorPubIPName -ErrorAction Stop
     $conductorPubIP=(Get-AzPublicIpAddress -ResourceGroupName $rgName -Name $conductorPubIPName).IpAddress 
     Write-Host "$(Get-Date)- IP address of conductor :"$conductorPubIP -ForegroundColor Yellow
    }
Catch {
       Write-Host "$(Get-Date)- IP address of conductor not found!" -ForegroundColor Yellow
       Exit
}
#>

$parameters=@{
              "adminUsername"= $ssrAdministratorUsername;
              "adminPublicKeyData"= $ssrPubRSAKey;
              "location"= $location;
              "conductorPrimaryControlIP"= ""
              }

Write-Host "$(Get-Date) - adminUsername............:"$ssrAdministratorUsername
Write-Host "$(Get-Date) - adminPublicKeyData.......:"$ssrPubRSAKey
Write-Host "$(Get-Date) - location.................:"$location
