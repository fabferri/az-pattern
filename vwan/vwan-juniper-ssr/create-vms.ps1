...
$armTemplateFile = 'create-vms.json'
$deploymentName = 'create-vms'
####################################################
...
Write-Host "$(Get-Date) - getting KeyVault Secret" -ForegroundColor Cyan
# fetch admin password from keyvault secret, specified in by name $adminUsername

$adminPassword = Get-AzKeyVaultSecret -VaultName $keyVaultName -Name $adminUsername -AsPlainText
if (!$adminPassword) { Write-Host 'variable $adminPassword is null' ; Exit } else { Write-Host "$(Get-Date) - administrator password: "$adminPassword -ForegroundColor Green}

If ($adminPassword -eq $null) { 
    Write-Host '  keyvault password not found!!!'
    Exit
}
...
$parameters=@{
              ...
              "vnet1location"= $location;
              "vnet2location"= $location;
              "vnet3location"= $location;
              "vnetSpoke1location"= $location
              }

Write-Host "$(Get-Date) - adminUsername...........:"$adminUsername -ForegroundColor Yellow
Write-Host "$(Get-Date) - adminPassword...........:"$adminPassword -ForegroundColor Yellow
Write-Host "$(Get-Date) - vnet1location...........:"$location -ForegroundColor Cyan
Write-Host "$(Get-Date) - vnet2location...........:"$location -ForegroundColor Cyan
Write-Host "$(Get-Date) - vnet3location...........:"$location -ForegroundColor Cyan
Write-Host "$(Get-Date) - vnetSpoke1location......:"$location -ForegroundColor Cyan
