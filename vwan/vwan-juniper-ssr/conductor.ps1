#
#  Deployment of Juniper Conductor (formerly 128T) by ARM template
#
# INPUT VARIABLES:
#  from file init.json
#
...
$parameters=@{
              "adminUsername" = $conductorAdministratorUsername;
              "adminPublicKeyData" = $conductorPubRSAKey;
...
Write-Host "$(Get-Date) - adminUsername.....:"$conductorAdministratorUsername -ForegroundColor Cyan
Write-Host "$(Get-Date) - adminPublicKeyData:"$conductorPubRSAKey -ForegroundColor Cyan
Write-Host "$(Get-Date) - location..........:"$conductorlocation -ForegroundColor Cyan
...
# set tag asosciated with the resource group
